...
const (
	...
	planModePromptHint             = "When plan mode is active, you MUST still use read tools (file_read, glob, grep, bash with read-only commands like ls/cat/find) and may use read-only child agents for bounded exploration. Only avoid write tools (create_file, file_write, file_edit, apply_patch, multi_replace_file_content). Use child agents as choreography, not orchestration: give them a focused objective, let them finish, and synthesize their result instead of polling unless they were explicitly launched in background. When you have a real implementation plan, save or update it with save_implementation_plan so it remains the primary review artifact for the task. Ask the user to review the plan, revise it in place when needed, and only move to /fast when they want implementation to begin."
)
