...
- Completed: Added hidden compatibility aliases for local model tool-name drift. The registry now accepts read-only aliases like `file_search` and `read_file` without advertising them in the tool definitions, so local models that invent Copilot-style names can still search and read files successfully.
...
- Completed: Added tool-name compatibility for local models that emit Copilot-style search/read calls. The engine now normalizes aliases such as `file_search`, `grep_search`, and `read_file` into the local tool set before execution, and the system prompt now names the exact supported tool identifiers explicitly.
