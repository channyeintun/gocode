...
- Completed: Fixed active-turn queueing and live-turn visibility in the TUI. Prompts submitted while another turn is still active now queue locally instead of clearing the current response, the live assistant row stays visible across tool execution and permission waits, `file_read` output renders as its own code preview row instead of disappearing into grouped exploration summaries, and Esc interruption now cancels active turns from the router immediately instead of waiting for the next message poll.
